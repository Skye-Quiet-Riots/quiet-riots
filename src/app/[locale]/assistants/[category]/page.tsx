  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-8 grid grid-cols-3 gap-3">
        <StatBadge value={detail.rioter_count} label={t('rioters')} />
        <StatBadge value={detail.riot_count} label={t('riots', { count: detail.riot_count })} />
        <StatBadge value={detail.messages_sent} label={t('messages')} />
      </div>
      ...
      {detail.focus && (
        <div className="mb-8 rounded-xl border border-zinc-200 bg-zinc-50 p-5 dark:border-zinc-700 dark:bg-zinc-800/50">
          ...
          {detail.focus_detail && (
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{detail.focus_detail}</p>
          )}
        </div>
      )}
      ...
      {detail.riots.length > 0 && (
        <div className="mb-8">
          <h2 className="mb-3 text-lg font-bold">{t('riotsWeHelp')}</h2>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-xs text-zinc-500 dark:text-zinc-400">
                    {riot.rioter_count.toLocaleString()} {t('rioters')}
                  </span>
                </div>
                {riot.agent_helps && (
                  <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                    <span className="font-medium">{detail.agent_name}:</span> {riot.agent_helps}
                  </p>
                )}
                {riot.human_helps && (
                  <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                    <span className="font-medium">{detail.human_name}:</span> {riot.human_helps}
                  </p>
                )}
              </Link>
            ))}
