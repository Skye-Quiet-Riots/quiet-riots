...
describe('updateUser', () => {
  it('updates specific fields', async () => {
    const updated = await updateUser('user-sarah', { name: 'Sarah Updated' });
  ...
  it('returns existing user when no fields provided', async () => {
    const user = await updateUser('user-sarah', {});
    expect(user).not.toBeNull();
    expect(user!.id).toBe('user-sarah');
  });

  it('returns null for missing user', async () => {
    const user = await updateUser('nonexistent', { name: 'Ghost' });
    expect(user).toBeNull();
  });
});
...
describe('getUserIssues', () => {
  it('returns joined issues with details', async () => {
    const issues = await getUserIssues('user-sarah');
    expect(issues.length).toBeGreaterThanOrEqual(1);
    // Sarah joined Rail Cancellations
    const railIssue = issues.find((i) => i.issue_id === 'issue-rail');
...
describe('joinIssue / leaveIssue / hasJoinedIssue', () => {
  it('joins a user to an issue', async () => {
    await joinIssue('user-sarah', 'issue-broadband'); // Sarah joins Broadband Speed
    const joined = await hasJoinedIssue('user-sarah', 'issue-broadband');
    expect(joined).toBe(true);
  });

  it('is idempotent (INSERT OR IGNORE)', async () => {
    // Should not throw on duplicate
    await joinIssue('user-sarah', 'issue-broadband');
    const joined = await hasJoinedIssue('user-sarah', 'issue-broadband');
    expect(joined).toBe(true);
  });

  it('leaves an issue', async () => {
    await leaveIssue('user-sarah', 'issue-broadband');
    const joined = await hasJoinedIssue('user-sarah', 'issue-broadband');
    expect(joined).toBe(false);
  });

  it('hasJoinedIssue returns false when not joined', async () => {
    const joined = await hasJoinedIssue('user-sarah', 'issue-flights');
    expect(joined).toBe(false);
  });
});
