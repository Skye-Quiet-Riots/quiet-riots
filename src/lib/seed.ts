...
export async function seed() {
  ...
  const issues = [
    ...
    [
      'Rail Cancellations',
      'Transport',
      'Frequent train cancellations causing commuter misery',
      2847,
      3,
      340,
    ],
    [
      'Train Ticket Prices',
      'Transport',
      'Rising rail fares outpacing inflation and wages',
      5612,
      4,
      180,
    ],
    [
      'Flight Delays',
      'Transport',
      'Chronic flight delays and inadequate compensation',
      12340,
      28,
      890,
    ],
    [
      'Lost Luggage',
      'Transport',
      'Airlines losing passenger baggage with slow resolution',
      8920,
      35,
      420,
    ],
    [
      'Bus Route Cuts',
      'Transport',
      'Rural and suburban bus services being reduced or eliminated',
      3210,
      3,
      150,
    ],
    ...
    [
      'Mobile Data Costs',
      'Telecoms',
      'Excessive mobile data charges and hidden fees',
      6780,
      12,
      340,
    ],
    [
      ...
      'Telecoms',
      'Unreasonable wait times when calling customer support',
      3450,
      8,
      210,
    ],
    ...
    [
      ...
      'Banking',
      'Excessive fees for international money transfers',
      7890,
      22,
      670,
    ],
    ...
    [
      'NHS Waiting Times',
      'Health',
      'Dangerously long waiting times for NHS appointments and treatments',
      8934,
      1,
      890,
    ],
    [
      ...
      'Health',
      'Inadequate funding and access to mental health support',
      4567,
      6,
      340,
    ],
    ...
    [
      ...
      'Education',
      'Curricula failing to prepare students for modern challenges',
      6780,
      18,
      560,
    ],
    [
      'Student Debt',
      'Education',
      'Crippling student loan burdens affecting life choices',
      11230,
      4,
      920,
    ],
    ...
    [
      'Climate Change',
      'Environment',
      'Insufficient action on climate change from governments and corporations',
      45670,
      82,
      3400,
    ],
    [
      'Plastic Waste',
      'Environment',
      'Excessive single-use plastics polluting oceans and ecosystems',
      23450,
      54,
      1800,
    ],
  ...
  // =============================
  const orgSql = `INSERT INTO organisations (id, name, category, logo_emoji, description) VALUES (?, ?, ?, ?, ?)`;
  ...
  await db.execute({
    sql: linkSql,
    args: [issueIds['Rail Cancellations'], orgIds['Southern Rail'], 2847, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Rail Cancellations'], orgIds['Deutsche Bahn'], 1203, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Rail Cancellations'], orgIds['SNCF'], 956, 3],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Rail Cancellations'], orgIds['Trenitalia'], 734, 4],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Rail Cancellations'], orgIds['Amtrak'], 612, 5],
  });

  // Train Ticket Prices
  await db.execute({
    sql: linkSql,
    args: [issueIds['Train Ticket Prices'], orgIds['Southern Rail'], 1923, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Train Ticket Prices'], orgIds['Deutsche Bahn'], 1456, 3],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Train Ticket Prices'], orgIds['SNCF'], 1102, 4],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Train Ticket Prices'], orgIds['Trenitalia'], 678, 5],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Train Ticket Prices'], orgIds['Amtrak'], 453, 6],
  });

  // Flight Delays
  await db.execute({
    sql: linkSql,
    args: [issueIds['Flight Delays'], orgIds['Southern Rail'], 890, 4],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Flight Delays'], orgIds['Deutsche Bahn'], 2340, 1],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Flight Delays'], orgIds['SNCF'], 1567, 2] });
  await db.execute({ sql: linkSql, args: [issueIds['Flight Delays'], orgIds['Amtrak'], 1234, 3] });

  // Lost Luggage
  await db.execute({
    sql: linkSql,
    args: [issueIds['Lost Luggage'], orgIds['Southern Rail'], 456, 5],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Lost Luggage'], orgIds['Deutsche Bahn'], 1890, 1],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Lost Luggage'], orgIds['SNCF'], 1234, 2] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Lost Luggage'], orgIds['Trenitalia'], 987, 3],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Lost Luggage'], orgIds['Amtrak'], 678, 4] });

  // Bus Route Cuts
  await db.execute({
    sql: linkSql,
    args: [issueIds['Bus Route Cuts'], orgIds['Southern Rail'], 1456, 3],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Bus Route Cuts'], orgIds['Deutsche Bahn'], 567, 5],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Bus Route Cuts'], orgIds['Amtrak'], 890, 4] });

  // Broadband Speed across telecoms
  await db.execute({ sql: linkSql, args: [issueIds['Broadband Speed'], orgIds['BT'], 2345, 1] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Broadband Speed'], orgIds['Vodafone'], 1234, 2],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Broadband Speed'], orgIds['Three'], 533, 3] });

  // Mobile Data Costs
  await db.execute({ sql: linkSql, args: [issueIds['Mobile Data Costs'], orgIds['BT'], 1890, 2] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Mobile Data Costs'], orgIds['Vodafone'], 2567, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Mobile Data Costs'], orgIds['Three'], 2323, 3],
  });

  // Customer Service Hold Times
  await db.execute({
    sql: linkSql,
    args: [issueIds['Customer Service Hold Times'], orgIds['BT'], 1567, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Customer Service Hold Times'], orgIds['Vodafone'], 1023, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Customer Service Hold Times'], orgIds['Three'], 860, 3],
  });

  // Banking issues
  await db.execute({
    sql: linkSql,
    args: [issueIds['Overseas Transfer Fees'], orgIds['HSBC'], 3456, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Overseas Transfer Fees'], orgIds['Barclays'], 2345, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Overseas Transfer Fees'], orgIds['NatWest'], 2089, 3],
  });

  await db.execute({ sql: linkSql, args: [issueIds['Hidden Charges'], orgIds['HSBC'], 4120, 1] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Hidden Charges'], orgIds['Barclays'], 2890, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Hidden Charges'], orgIds['NatWest'], 2110, 3],
  });

  await db.execute({
    sql: linkSql,
    args: [issueIds['Bank Branch Closures'], orgIds['HSBC'], 890, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Bank Branch Closures'], orgIds['Barclays'], 823, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Bank Branch Closures'], orgIds['NatWest'], 627, 3],
  });

  // Health issues
  await db.execute({
    sql: linkSql,
    args: [issueIds['NHS Waiting Times'], orgIds['NHS England'], 8412, 1],
  });
  await db.execute({ sql: linkSql, args: [issueIds['NHS Waiting Times'], orgIds['NICE'], 312, 2] });
  await db.execute({ sql: linkSql, args: [issueIds['NHS Waiting Times'], orgIds['PHE'], 210, 3] });

  await db.execute({
    sql: linkSql,
    args: [issueIds['Mental Health Services'], orgIds['NHS England'], 3456, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Mental Health Services'], orgIds['NICE'], 678, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Mental Health Services'], orgIds['PHE'], 433, 3],
  });

  await db.execute({
    sql: linkSql,
    args: [issueIds['Prescription Costs'], orgIds['NHS England'], 2345, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Prescription Costs'], orgIds['NICE'], 534, 2],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Prescription Costs'], orgIds['PHE'], 331, 3] });

  // Education issues
  await db.execute({
    sql: linkSql,
    args: [issueIds['Outdated Teaching Methods'], orgIds['Department for Education'], 3456, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Outdated Teaching Methods'], orgIds['Ofsted'], 2134, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Outdated Teaching Methods'], orgIds['Student Loans Company'], 1190, 3],
  });

  await db.execute({
    sql: linkSql,
    args: [issueIds['Student Debt'], orgIds['Department for Education'], 4567, 1],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Student Debt'], orgIds['Student Loans Company'], 5123, 2],
  });
  await db.execute({ sql: linkSql, args: [issueIds['Student Debt'], orgIds['Ofsted'], 1540, 3] });

  await db.execute({
    sql: linkSql,
    args: [issueIds['School Funding'], orgIds['Department for Education'], 2890, 1],
  });
  await db.execute({ sql: linkSql, args: [issueIds['School Funding'], orgIds['Ofsted'], 1567, 2] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['School Funding'], orgIds['Student Loans Company'], 883, 3],
  });

  // Environment issues
  await db.execute({ sql: linkSql, args: [issueIds['Climate Change'], orgIds['DEFRA'], 18900, 1] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Climate Change'], orgIds['Environment Agency'], 15670, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Climate Change'], orgIds['Forestry Commission'], 11100, 3],
  });

  await db.execute({ sql: linkSql, args: [issueIds['Plastic Waste'], orgIds['DEFRA'], 10230, 1] });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Plastic Waste'], orgIds['Environment Agency'], 8450, 2],
  });
  await db.execute({
    sql: linkSql,
    args: [issueIds['Plastic Waste'], orgIds['Forestry Commission'], 4770, 3],
  });
  ...
  const synonyms: [string, string[]][] = [
    [
      'Rail Cancellations',
      ['train cancellations', 'cancelled trains', 'rail disruptions', 'service cancellations'],
    ],
    ...
    [
      'Overseas Transfer Fees',
      ['international transfer costs', 'remittance fees', 'wire transfer charges'],
    ],
  ...
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Write to Transport Regulator',
    'Send a formal complaint using our community template',
    'action',
    '10min',
    'writing',
  ...
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Claim your refund',
    'Get compensation for delayed/cancelled trains',
    'action',
    '1min',
    '',
    'https://www.southernrailway.com/help-and-contact/delay-repay',
    'Delay Repay',
  ]);
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Sign the petition',
    'Join 4,231 others demanding better service',
    'action',
    '1min',
    '',
  ...
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Film your platform',
    'Document overcrowding and cancellation boards',
    'action',
    '10min',
    'media',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Join the community brainstorm session',
    'idea',
    '10min',
    '',
    null,
  ...
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Switch provider',
    'Compare alternative routes and operators',
    'action',
    '1min',
    '',
  ...
  await insertRow(actionSql, [
    issueIds['Rail Cancellations'],
    'Celebrate wins',
    'Recognise when progress is made',
    'together',
    '1min',
    '',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Tell others about your cancellation stories',
    'idea',
    '10min',
    'writing',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Speed test evidence',
    'Document your actual vs advertised speeds',
    'action',
    '1min',
    'tech',
  ...
  await insertRow(actionSql, [
    issueIds['Broadband Speed'],
    'Complain to Ofcom',
    'File a formal complaint with the regulator',
    'action',
    '10min',
    'writing',
  ...
  await insertRow(actionSql, [
    issueIds['Broadband Speed'],
    'Share your speed data',
    'Contribute your speed test results',
    'idea',
    '1min',
    'tech',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Help others optimise their setup',
    'together',
    '10min',
    'tech',
    null,
  ...
  await insertRow(actionSql, [
    issueIds['Broadband Speed'],
    'Vote on priorities',
    'Help decide which campaign to focus on',
    'idea',
    '1min',
    '',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Write to your MP',
    'Use our template to contact your representative',
    'action',
    '10min',
    'writing',
  ...
  await insertRow(actionSql, [
    issueIds['NHS Waiting Times'],
    'Sign the NHS petition',
    'Join 12,000+ calling for more funding',
    'action',
    '1min',
    '',
    'https://petition.parliament.uk',
  ...
  await insertRow(actionSql, [
    ...
    'Your experience helps build the case for change',
    'idea',
    '10min',
    'writing',
    null,
  ...
  await insertRow(actionSql, [
    issueIds['NHS Waiting Times'],
    'Attend virtual rally',
    'Join the online campaign event this Saturday',
    'together',
    '1hr+',
    'organising',
    null,
  ...
  await insertRow(actionSql, [
    issueIds['NHS Waiting Times'],
    'Translate campaign materials',
    'Help reach non-English speaking communities',
    'together',
    '1hr+',
    'languages',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Help us map wait times across the country',
    'idea',
    '10min',
    'tech',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Help the community compare bank charges',
    'together',
    '1hr+',
    'tech',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Calculate and share your household footprint',
    'idea',
    '10min',
    '',
  ...
  await insertRow(actionSql, [
    issueIds['Climate Change'],
    'Join local climate group',
    'Connect with activists in your area',
    'together',
    '1hr+',
    'organising',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Join 1M+ calling for net zero by 2035',
    'action',
    '1min',
    '',
    'https://petition.parliament.uk',
  ...
  await insertRow(actionSql, [
    ...
    'Plan a local awareness or action day',
    'together',
    '1hr+',
    'organising,media',
    null,
  ...
  await insertRow(actionSql, [
    ...
    'Help others manage their student debt',
    'together',
    '10min',
    'writing',
    null,
  ...
  await insertRow(actionSql, [
    issueIds['Student Debt'],
    'Brainstorm policy alternatives',
  ...
  await insertRow(actionSql, [
    issueIds['Plastic Waste'],
    'Support plastic ban petition',
    'Sign the petition for single-use plastic legislation',
    'action',
    '1min',
    '',
    'https://petition.parliament.uk',
  ...
  for (const issueName of remainingIssues) {
    ...
    await insertRow(actionSql, [
      issueIds[issueName],
      'Write to your representative',
      'Use our template to demand change',
      'action',
      '10min',
      'writing',
      'https://www.writetothem.com',
    ...
    await insertRow(actionSql, [
      issueIds[issueName],
      'Vote on priorities',
      'Help decide what to focus on next',
      'idea',
      '1min',
      '',
      null,
  ...
  await insertRow(expertSql, [
    ...
    'Hans W.',
    'Green Energy Expert',
    'DE/EN · Renewable policy',
    'Advised 3 local councils on net zero',
    '🌱',
  ]);
  ...
  await insertRow(feedSql, [
    issueIds['Climate Change'],
    userIds['Carlos M.'],
    'Translated the climate action toolkit into Spanish. 6 more languages to go! 🌍',
    42,
    '2026-02-15 08:00:00',
  ]);
  await insertRow(feedSql, [
    issueIds['Climate Change'],
    userIds['Priya S.'],
    'Our local council just committed to net zero by 2030. The campaign worked!',
    103,
    '2026-02-14 11:00:00',
  ]);
  ...
  // Fixed rioter counts for remaining issues (no longer derived from integer IDs)
  const basicCountryData: Record<string, [number, number, number]> = {
    'Train Ticket Prices': [3800, 1200, 580],
    'Bus Route Cuts': [2400, 800, 390],
    'Mobile Data Costs': [3600, 1150, 550],
    'Customer Service Hold Times': [2800, 950, 460],
    'Overseas Transfer Fees': [4200, 1350, 650],
    'Bank Branch Closures': [1800, 650, 320],
    'Mental Health Services': [3200, 1050, 510],
    'Prescription Costs': [2500, 850, 410],
    'Outdated Teaching Methods': [3700, 1200, 570],
    'School Funding': [3100, 1000, 490],
  };
  for (const [issueName, [gb, us, de]] of Object.entries(basicCountryData)) {
    await insertRow(countrySql, [issueIds[issueName], 'GB', 'United Kingdom', gb]);
    await insertRow(countrySql, [issueIds[issueName], 'US', 'United States', us]);
    await insertRow(countrySql, [issueIds[issueName], 'DE', 'Germany', de]);
  }
  ...
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Sarah K.'], issueIds['Rail Cancellations']],
  });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Marcio R.'], issueIds['Rail Cancellations']],
  });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['James L.'], issueIds['Rail Cancellations']],
  });
  await db.execute({ sql: userIssueSql, args: [userIds['James L.'], issueIds['Broadband Speed']] });
  await db.execute({ sql: userIssueSql, args: [userIds['Yuki T.'], issueIds['Broadband Speed']] });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Dr. Patel'], issueIds['NHS Waiting Times']],
  });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Priya S.'], issueIds['NHS Waiting Times']],
  });
  await db.execute({ sql: userIssueSql, args: [userIds['Priya S.'], issueIds['Climate Change']] });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Emma W.'], issueIds['NHS Waiting Times']],
  });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Emma W.'], issueIds['Rail Cancellations']],
  });
  await db.execute({ sql: userIssueSql, args: [userIds['Carlos M.'], issueIds['Climate Change']] });
  await db.execute({
    sql: userIssueSql,
    args: [userIds['Carlos M.'], issueIds['Rail Cancellations']],
  });
  ...
  console.log(
    '   Actions, experts, community health, feed posts, countries, synonyms — all populated',
  );
}
