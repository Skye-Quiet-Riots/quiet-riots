...
> ```
> Then re-read this file (CLAUDE.md) from the updated worktree. If you skip this step, you WILL read stale session docs and miss the previous session's work. This has happened in sessions 71 AND 74 — both times the agent read SESSION_LOG.md as its very first action before fetching, saw outdated session info, and wasted context on already-completed work.
>
> **DO NOT read SESSION_LOG.md before completing this sync.** The urge to "just check where we left off" is the exact failure mode. Sync first, read second.
...
- **Stale worktree SESSION_LOG.md causes wasted context:** When the session starts, SESSION_LOG.md in the worktree still points to whenever the worktree was created. Reading it before `git fetch origin main && git merge origin/main --no-edit` causes the agent to plan work that was already done in previous sessions. This happened in sessions 71 and 74. The fix: ALWAYS fetch+merge before reading ANY file. The MANDATORY FIRST ACTION block at the top of this file exists specifically for this.
...
4. **MANDATORY: Merge ALL session docs PRs before session ends.** Check CI (`gh pr checks <number> --watch`), then merge: `gh pr merge <number> --squash --admin` or `gh api repos/Skye-Quiet-Riots/quiet-riots/pulls/<number>/merge -X PUT -f merge_method=squash`. A session docs PR left unmerged is invisible to the next session — it cannot read session files that aren't on `origin/main`. This is the #1 cause of "session N docs were lost" bugs.
5. **Merge ALL open PRs:** Run `gh pr list --state open`. Handle each:
   - **Our PRs** (`--author Simon-Quiet-Riots`): Wait for CI (`gh pr checks <number> --watch`), then merge: `gh pr merge <number> --squash --admin`. The `--admin` flag bypasses the 1-approval branch protection rule. If `gh pr merge` fails in a worktree ("`main` already used"), use: `gh api repos/Skye-Quiet-Riots/quiet-riots/pulls/<number>/merge -X PUT -f merge_method=squash`. **Never leave PRs open across sessions** — the next session cannot see unmerged branches.
